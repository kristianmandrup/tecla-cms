  ...
  def set_hash
    ...
    composite_hash_items.each do |item|
      ...
      if item._type == "Cms::HashList"
        ...
        hash[item.name] = item.blocks.collect{ |block| block.as_json(only: [:name])}
      elsif item._type == "Cms::ImageList"
        hash[item.name] = item.images.collect{ |block| block.as_json(only: [:name])}
      elsif item._type == "Cms::NamedBlock"
        hash[item.name] = item.block.as_json(only: [:name])
      elsif item._type == "Cms::NamedImage"
        hash[item.name] = item.image.as_json(only: [:name])
      end
    end
    hash
  end
